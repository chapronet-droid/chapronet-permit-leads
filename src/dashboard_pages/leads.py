...
from dashboard_common import load_leads, money, page_header, empty_state, paginate, priority_badge, render_tags_html, compute_lead_tags
...
def render() -> None:
    ...
    min_score = st.slider("Minimum lead score", 0, 100, 55, key="leads_min_score")
    qualified = df[df.get("lead_score", 0).fillna(0) >= min_score].sort_values("lead_score", ascending=False)
            ...
            st.markdown(
                ...
                <div style="padding:0.75rem 0; border-bottom:1px solid #E2E8F0;">
                  <div style="display:flex; justify-content:space-between; align-items:center;">
                    ...
                    <div>{priority_badge(row.get('priority',''))} <span class="badge badge-blue">Score {int(row.get('lead_score', 0) or 0)}/100</span></div>
                  </div>
